...
class AgentModeDaemon:
    ...
    def get_train_data_batch(
        ...
        rollout_log_probs_list: List[List[float]] = []
        ...
        if self.trace_aggregator.get("level", "transition") == "transition":
                ...
                for turn_index, trace in enumerate(sample_info["trace_list"]):
                    ...
                    if rollout_log_probs:
                        if len(rollout_log_probs) > max_response_length:
                            one_rollout_log_probs = rollout_log_probs[:max_response_length]
                        else:
                            one_rollout_log_probs = rollout_log_probs + [0.0] * (max_response_length - len(rollout_log_probs))
                    else:
                    ...
                    if one_rollout_log_probs:
                        rollout_log_probs_list.append(one_rollout_log_probs)
            ...
            for rollout_id, sample_info in finished_id_to_sample_info.items():
                ...
                for current_merged_trace_idx in merged_trace_idx:
                    ...
                    image_grid_thw = self._get_image_grid_thw(
                        sample_info["trace_list"][current_merged_trace_idx[0]].get("image_urls", [])) if self._use_mrope else None
