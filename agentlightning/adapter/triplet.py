...
class LlmProxyTraceToTriplet(TraceToTripletBase):
    ...
    def adapt(self, source: Sequence[Span], /) -> List[Triplet]:  # type: ignore
        ...
        for s in spans:
            ...
            prompt_ids: List[int] = []
            resp_ids: List[int] = []
            resp_logprobs: List[dict] = []
            prompt_raw_content: List[dict] = []

            if s.name == "raw_gen_ai_request":
                prompt_ids, resp_ids, resp_logprobs, prompt_raw_content = self._extract_tokens_from_raw(attrs)
            elif s.name == "litellm_request":
                # Some proxies never include token ids here. Ignore unless present.
                prompt_ids, resp_ids, resp_logprobs, prompt_raw_content = self._extract_tokens_from_openai(attrs)
            ...
            if prompt_ids and resp_ids:
                ...
                llm_items.append(
                    dict(
                        ...
                        response_logprobs=resp_logprobs,
                        request_id=rid,
